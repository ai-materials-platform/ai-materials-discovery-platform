...
| `/features/available` | GET | 전처리 후 선택 가능한 피처 컬럼 목록 |
...
| 18-1 | 피처 선택 패널 (전처리 완료 후 활성화, 체크박스 목록) | `renderer/pages/preprocess.js` |
| 19 | 학습 탭 (SSE 연동, 선택된 피처 수 요약 표시) | `renderer/pages/training.js` |
...
- [ ] 전처리 탭: 전처리 완료 후 피처 선택 패널 활성화, 체크박스 선택/해제 정상 동작
- [ ] 학습 탭: 선택된 피처 수 요약 표시 확인
